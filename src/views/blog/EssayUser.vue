<template>
    <div style="margin-left: 10px;margin-right: 10px;margin-top:30px">
        ...
        <div style="margin-top:20px">
            <el-card class="box-card" v-for="blog in essayFrom"
                :key="blog.id" shadow="never">
                <div class="text item" style="color:#999AAA;">
                    <el-row>
                        <el-col :span="8" @click="getBlog(blog.blogId)" style="cursor: pointer;">
                            <span v-if="blog.picUrl"><img :src="blog.picUrl" style="width: 100%;height: 100%;" alt /></span>
                        </el-col>
                        <el-col :span="16">
                            <el-row style="margin-left:10px">
                                <el-col :span="6" @click="getBlog(blog.blogId)" style="cursor: pointer;">
                                ...
                                <el-col :span="3" style="margin: auto;">
                                    <i class="el-icon-position" style="color:#999AAA;cursor: pointer;" @click="toEditor(blog.blogId)" >编辑</i>
                                </el-col>
                                <el-col :span="3" style="margin: auto;">
                                    <i class="el-icon-delete" style="color:#999AAA;cursor: pointer;" @click="deleteEssayById(blog.blogId)" >删除</i>
                                </el-col>
                            </el-row>
                            <el-row style="height:100%">
                                <el-col :span="12" @click="getBlog(blog.blogId)" style="cursor: pointer;">
...
import { getHomeUserDec,getUserAllEssay,deleteEssay } from "../../api/index";
...
export default {
    ...
    setup() {
        ...
        const essayFrom = ref([]);
        const getUserEssay = ()=>{
            getUserAllEssay().then(res=>{
                if(res.errorCode === 200){
                    essayFrom.value = res.data;
                }
            })
        };
        const getBlog = (id)=>{
        ...
        const toEditor = (id)=>{
            router.push({path:'/editor',query:{blogId:id}});
        };
        const deleteEssayById = (id)=>{
            query.id = id;
            deleteEssay(query).then(res=>{
                if(res.errorCode === 200){
